...
#median_down_pd
newselect = newselect.loc[pandas_df["closing_id"]==2]#.T
newselect_transpose = newselect.T#["asset_id"]
newpdf = newselect_transpose['asset_id':].head(2)
#newselect_transpose = newselect[["asset_id"]]



#newselect = np.array(newselect)
#newselect = newselect.transpose()
#newselect = newselect.toPandas()
#newselect = transpose(newselect)


selection = np.zeros((investment_universe_month.shape[0],10))

for q in range(1,investment_universe_month.shape[0]):
    newselect = newselect.loc[pandas_df["closing_id"]==q]#.T
    newselect_transpose = newselect.T#["asset_id"]
    newpdf = newselect_transpose['asset_id':].head(2)
    selection[q] = newpdf
    ...
    #    if pandas_df["closing_id"].any() == q:
#        selection[q] = pandas_df.loc[pandas_df["closing_id"]==q]
